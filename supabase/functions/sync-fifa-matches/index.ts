...
const SYNC_VERSION = 'safe-goal-events-20260618-v4';
...
Deno.serve(async () => {
  ...
  try {
    ...
    const { error: deleteOldEventsError } = await supabase
      ...
      .delete()
      .neq('event_key', '__never__');
    assertNoError(deleteOldEventsError, 'match_goal_events full refresh delete');
    ...
    const fifaGoalRows = mergePlayerGoalRows(aggregateGoalEvents((storedGoalEvents || []) as GoalEventRow[]));
    let mergedGoals = fifaGoalRows;
    let externalScorers = 0;
    let externalSyncNote: string | null = null;
    let externalSource = 'none';

    if (!mergedGoals.length) {
      const fallbackSync = await fetchEspnEventGoalRows(matchRows);
      mergedGoals = mergePlayerGoalRows(fallbackSync.rows);
      externalScorers = fallbackSync.rows.length;
      externalSyncNote = fallbackSync.note;
      externalSource = fallbackSync.source;
    }

    fifaScorers = fifaGoalRows.length;
    scorerSyncNote = fifaGoalRows.length
      ? 'Fuente única: eventos FIFA por partido validados contra marcador'
      : externalSyncNote || 'FIFA no ha publicado eventos de gol utilizables y ESPN no devolvió eventos seguros';

    await replacePlayerGoals(supabase, mergedGoals);
    scorerSource = fifaGoalRows.length ? 'fifa-events' : mergedGoals.length ? externalSource : 'none';
...
function extractGoalEvents(match: any, matchId: string, home: NormalizedTeam, away: NormalizedTeam, matchNumber: number | null): GoalEventRow[] {
  ...
  const readGoals = (items: unknown[], source: string, side?: 'home' | 'away') => {
    ...
    items.forEach((item, index) => {
      ...
      const eventKey = [
        ...
        team?.code || team?.id || 'unknown',
        minute ?? index,
        penalty ? 'penalty' : 'open',
        ownGoal ? 'own' : 'regular',
      ].join(':');
  ...
  const expectedHome = nullableNumber(match.HomeTeamScore);
  const expectedAway = nullableNumber(match.AwayTeamScore);
  const expectedTotal = expectedHome !== null && expectedAway !== null ? expectedHome + expectedAway : null;
  const explicitRows = [
    ...readGoals([match.Home?.Goals, match.Home?.GoalEvents, match.HomeGoals, match.HomeTeamGoals].flatMap(asArray), 'home-goals', 'home'),
    ...readGoals([match.Away?.Goals, match.Away?.GoalEvents, match.AwayGoals, match.AwayTeamGoals].flatMap(asArray), 'away-goals', 'away'),
  ];

  if (isPlausibleGoalSet(explicitRows, expectedTotal, expectedHome, expectedAway, home, away)) return explicitRows;

  const eventCollections = [
    match.MatchEvents,
    match.Events,
    match.Incidents,
    match.MatchIncidents,
    match.Timeline,
    match.MatchFacts,
  ];

  for (const collection of eventCollections) {
    const rows = readGoals(asArray(collection), 'match-events');
    if (isPlausibleGoalSet(rows, expectedTotal, expectedHome, expectedAway, home, away)) return rows;
  }

  return [];
}

function isPlausibleGoalSet(
  rows: GoalEventRow[],
  ...
  if (expectedTotal !== null && rows.length > expectedTotal) return false;

  const homeGoals = rows.filter((row) => row.team_id === home?.id || row.team_code === home?.code).length;
  const awayGoals = rows.filter((row) => row.team_id === away?.id || row.team_code === away?.code).length;
  if (expectedHome !== null && homeGoals > expectedHome) return false;
  if (expectedAway !== null && awayGoals > expectedAway) return false;
...
async function fetchEspnEventGoalRows(matchRows: any[]) {
  const errors: string[] = [];
  const eventScorers = await fetchEspnEventScorers(errors, matchRows);
  const rows = mergeExternalScorers(eventScorers.filter((row) => row.goals > 0));
  if (rows.length) {
    return {
      source: 'espn-events',
      rows,
      note: errors.length
        ? `Fuente única: eventos ESPN verificados contra calendario FIFA; avisos: ${errors.slice(0, 3).join(' | ')}`
        : 'Fuente única: eventos ESPN verificados contra calendario FIFA',
    };
  }
  ...
  return {
    ...
    note: errors.length ? `Sin eventos de gol seguros: ${errors.slice(0, 3).join(' | ')}` : 'Sin eventos de gol seguros',
  };
}
...
async function fetchEspnEventScorers(errors: string[], matchRows: any[]) {
  try {
    ...
    const events = asArray(scoreboard?.events)
      ...
      .filter(Boolean) as EspnEventContext[];
...
function buildFifaMatchMatcher(matchRows: any[]) {
  ...
  matchRows.forEach((match) => {
    ...
    const homeScore = nullableScore(match.home_score);
    const awayScore = nullableScore(match.away_score);
    const expectedGoals = homeScore !== null && awayScore !== null ? homeScore + awayScore : null;
...
function readEspnEventContext(
  ...
  const pairKey = teamPairKey(teamCodes);
...
function readEspnSummaryScorers(summary: any) {
  ...
  const plays = playGroups.find((group) => group.some((play) => readEspnGoalPlay(play))) || [];

  plays.forEach((play, index) => {
    const row = readEspnGoalPlay(play);
    if (!row) return;
...
function readEspnGoalPlay(play: any): ExternalScorer | null {
...
async function replacePlayerGoals(supabase: any, rows: PlayerGoalRow[]) {
  ...
  const incomingNames = new Set(rows.map((row) => normalizeText(row.player_name)));
  const { data, error: selectError } = await supabase.from('player_goals').select('player_key, player_name');
  assertNoError(selectError, 'player_goals select before scorer replace');

  for (const current of data || []) {
    if (incomingKeys.has(current.player_key)) continue;
    if (incomingNames.has(normalizeText(current.player_name))) {
      const { error: deleteError } = await supabase.from('player_goals').delete().eq('player_key', current.player_key);
      assertNoError(deleteError, `player_goals duplicated scorer delete ${current.player_key}`);
      continue;
    }
    const { error: deleteError } = await supabase.from('player_goals').delete().eq('player_key', current.player_key);
  ...
  assertNoError(error, 'player_goals scorer upsert');
}
...
function normalizeTeamCode(code: string | null, name: string | null) {
  ...
  const upperCode = rawCode ? TEAM_CODE_ALIAS[rawCode] || rawCode : null;
