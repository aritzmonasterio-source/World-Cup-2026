...
drop policy if exists "community_memberships_read_approved_scope" on public.community_memberships;
create policy "community_memberships_read_approved_scope" on public.community_memberships
for select to authenticated
using (
  user_id = auth.uid()
...
drop policy if exists "community_settings_read_approved_scope" on public.community_settings;
create policy "community_settings_read_approved_scope" on public.community_settings
for select to authenticated
using (public.has_community_membership(community_id));
...
create policy "point_events_read_approved_scope" on public.point_events
for select to authenticated
using (
  (user_id = auth.uid() and public.has_community_membership(community_id))
  or public.is_community_admin(community_id)
